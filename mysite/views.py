...
def extension_google_login_redirect(request):
     return redirect(f'{settings.BACKEND_API_URL}/auth/google/login/extension')
...
def extension_google_login_callback(request):
    token = request.GET.get('token')
    user_id = request.GET.get('user_id')
...
def feedback_extension(request):
    return render(request,'feedback_extension.html')